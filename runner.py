...
# import use_gpu # for gpu support - unneccesary for now
...
class Runner:
    def __init__(self, args, verbose: int=1) -> None:
        ...
        # self.target_state = [1.] + [0.] * ( (1<<(self.args.qbits)) - 1 )
        # self.target_state /= np.linalg.norm(self.target_state)
        # self.target_state = stim.Tableau.from_state_vector(self.target_state, endian='little')
        ...
        utils._globals['wandb'] = self.args.wandb
    ...
    def make_expt(self):
        ...
        if self.training_req:
            self.exp.initialize_env(self.target_state, self.args.tol, self.args.maxsteps, self.seed, training_req=self.training_req, n_workers=self.args.numworkers, num_envs=utils._globals['num_envs'])
    ...
    def main(self) -> Union[list, None]:
        ...
        if self.training_req and utils._globals['wandb']:
            import wandb
            wandb.init(project='aalto', name=self.path[6:], config=vars(self.args))
        try:
            for seed in self.args.seed:
                ...
                if self.args.ctrain:
                    ...
                    if self.args.dist.startswith('bounded'): 
                        ...
                        kwargs['gateset'] = list(Environment.prepare_gatelist(self.args.qbits)[-1].values())
                        print(kwargs['gateset'])
                    elif self.args.dist.startswith('brickwork'):
                        kwargs['depth'] = int(self.args.dist[9:])

                    test_set = utils.prepare_testbench_tableau(self.args.numeps, self.args.qbits, self.args.testfile, self.seed, overwrite=False, **kwargs)
                    results = []
                    for target in tqdm.tqdm(test_set):
                        # print('START', self.target_state)
                        # print('TARGET', target)
                        env = self.exp.initialize_test_env(self.target_state, target, self.args.tol, self.args.maxsteps)
            ...
            if self.training_req:
                ...
                return
                minsize = min(len(res) for res in train_results)
                print(minsize)
                train_results = [res[:minsize] for res in train_results]
                train_results_num_eps_first = list(zip(*train_results))
                print(np.array(train_results_num_eps_first).shape)
                self.exp.get_stats(train_results_num_eps_first, msg=f'train results, averaged over seeds {self.args.seed}', filesuffix='finaltrain')
...
#         [gate, params] = action.split('(')
#         match gate:
#             case 'sdg'|'tdg': inv_actions.append(gate[0] + '(' + params)
#             case _: inv_actions.append(action)
#     return inv_actions

# from qiskit import QuantumCircuit

# def make_ckt(qubits, actions):
#     ckt = QuantumCircuit(qubits)
#     for action in actions:
#         [gate, params] = action.split('(')
#         params = eval(f'({params}')
#         getattr(ckt, gate)(*params)
#     ckt.barrier()
#     return ckt
...
if __name__ == '__main__':
    ...
    os.system(f'python random_testbench.py -hyp "{args.qbits},{args.tol},{args.name},{args.exptdate}" -n 50 -name "random-test-{args.qbits}q"')