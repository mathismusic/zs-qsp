...
def gen_all(params: str, n, test_name, seed, dist, verbose=0):
    ...
    # args = old_args
    # with open(os.path.join(runner.path, 'hyper-params.txt'), 'w') as f:
    #     f.write(str(args))
    # # dump the json back also
...
def test(params: str, n, test_name, seed, dist, verbose=0, just_qiskit=0):
    ...
    # run qiskit function on the same testfile

    # run the stim circuit on the same testfile
    # import stim

    # def stim_circuit(stabilizer_generators: list[str]):
    #     return stim.Tableau.from_stabilizers(stabilizer_generators).to_circuit().to_file

    # test_set = utils.prepare_testbench_tableau(args.numeps, args.qbits, args.testfile, args.seed, overwrite=False)
    acts_ag = []
    acts_bm = []
    acts_bm_greedy = []
    ...
    # if not just_qiskit:
    #     print_circuits(args.qbits, results[0][:10], generators_bigidx_first[:10])
...
def print_circuits(qbits: int, results: list, generators: list):
    ...
    qc_hsdgh.sdg(0)
    qc_hsdgh.h(0)
    qc_hsdgh = qc_hsdgh.to_gate(label='HSdgH')
    print(gates)
